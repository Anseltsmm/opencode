...
func (s *PersistentShell) execCommand(command string, timeout time.Duration, ctx context.Context) commandResult {
	...
	tempDir := os.TempDir()
	stdoutFile := filepath.Join(tempDir, fmt.Sprintf("azkia-stdout-%d", time.Now().UnixNano()))
	stderrFile := filepath.Join(tempDir, fmt.Sprintf("azkia-stderr-%d", time.Now().UnixNano()))
	statusFile := filepath.Join(tempDir, fmt.Sprintf("azkia-status-%d", time.Now().UnixNano()))
	cwdFile := filepath.Join(tempDir, fmt.Sprintf("azkia-cwd-%d", time.Now().UnixNano()))
