...
import (
	...
	toolsPkg "github.com/Anseltsmm/azkia/internal/llm/tools"
...
func (c *copilotClient) exchangeGitHubToken(githubToken string) (string, error) {
	...
	req.Header.Set("User-Agent", "AZKIA/1.0")
...
func newCopilotClient(opts providerClientOptions) CopilotClient {
	...
	if copilotOpts.bearerToken != "" {
		...
		if githubToken == "" {
			logging.Error("GitHub token is required for Copilot provider. Set GITHUB_TOKEN environment variable, configure it in azkia.json, or ensure GitHub CLI/Copilot is properly authenticated.")
	...
	openaiClientOptions = append(openaiClientOptions,
		option.WithHeader("Editor-Version", "AZKIA/1.0"),
		option.WithHeader("Editor-Plugin-Version", "AZKIA/1.0"),
